...
function file_get_html($url, $arr_ch_opt=array(),$use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    // We DO force the tags to be terminated.
    $dom = new simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
    // For sourceforge users: uncomment the next line and comment the retreive_url_contents line 2 lines down if it is not already done.
//     $contents = file_get_contents($url, $use_include_path, $context, $offset);
    $contents = simple_curl_get($url,60,$arr_ch_opt);
...
function simple_curl_get($url, $second=60,$arr_ch_opt=array()){
	if(empty($url)){
		return false;
	}
	$arr_agent = array(
	...
	if(!empty($arr_ch_opt)){
		if(isset($arr_ch_opt['proxy']) && !empty($arr_ch_opt['proxy'])){
			curl_setopt($ch, CURLOPT_PROXY, $arr_ch_opt['proxy']);
		}
	}
	$data = curl_exec($ch);
	curl_close($ch);
